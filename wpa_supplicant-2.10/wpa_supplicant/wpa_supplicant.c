...
static int select_driver(struct wpa_supplicant *wpa_s, int i)
{
	...
	if (wpa_drivers[i]->global_init && global->drv_priv[i] == NULL) {
		global->drv_priv[i] = wpa_drivers[i]->global_init(global);   // global结构体的priv data获取
				     ...
				     const char *name)
{
	...
	if (name == NULL) {
		...
		for (i = 0; wpa_drivers[i]; i++) {
			if (select_driver(wpa_s, i) == 0)	// 调用global_init接口，成功就表示匹配上
				return 0;
		}
				     ...
				     const struct wpa_interface *iface)
{
	 ...
	 * call. */
	// 选择对应的驱动接口nl80211, wext函数，设置私有数据
	...
	if (capa_res == 0) {
		...
		wpa_s->max_scan_ssids = capa.max_scan_ssids;	// 扫描ssid个数
		wpa_s->max_sched_scan_ssids = capa.max_sched_scan_ssids;       
		wpa_s->max_sched_scan_plans = capa.max_sched_scan_plans;
		wpa_s->max_sched_scan_plan_interval =           // 扫描间隔
			capa.max_sched_scan_plan_interval;
		wpa_s->max_sched_scan_plan_iterations =
			capa.max_sched_scan_plan_iterations;
		wpa_s->sched_scan_supported = capa.sched_scan_supported;
		wpa_s->max_match_sets = capa.max_match_sets;
		wpa_s->max_remain_on_chan = capa.max_remain_on_chan;	// 信道停留时间
